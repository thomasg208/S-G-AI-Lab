        ...
        <div className="col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8">
          {data.columns.map((column, idx) => (
            <div key={idx} className="space-y-4">
              <h3 className="text-sm font-medium text-gray-400 uppercase tracking-wider">{column.title}</h3>
                ...
                {column.items.map((item, itemIdx) => (
                  <li key={itemIdx}>
                    ...
                    >
                      ...
                      <div>
                        <div className="font-medium text-white group-hover:text-purple-400">{item.title}</div>
